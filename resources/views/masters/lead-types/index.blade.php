...
<div class="flex-1 overflow-y-auto p-5 space-y-4 pb-20 bg-[#f8fafc]">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <span class="text-blue-600">Lead Type</span>
            </nav>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Lead Type Master</h1>
        </div>
        @can('lead-types.create')
        <button onclick="openAddModal()" class="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all flex items-center gap-2">
            <span class="material-symbols-outlined text-[18px] font-bold">add</span> Add Lead Type
        </button>
        @endcan
    </div>
    ...
    <div class="glass-panel rounded-[1.5rem] overflow-hidden">
        ...
        <div class="px-4 overflow-x-auto">
            <table class="w-full" id="lead-type-table">
                <thead>
                    <tr class="text-left">
                        ...
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Lead Type Name</th>
...
<div id="leadTypeModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/0 backdrop-blur-0 transition-all duration-300 opacity-0 pointer-events-none">
    <div class="modal-content glass-panel w-full max-w-sm rounded-[1.25rem] p-6 shadow-2xl transition-all duration-300 transform scale-95 opacity-0">
        <form id="leadTypeForm">
            ...
            <input type="hidden" id="leadType_id" name="id">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg font-black text-slate-800" id="modalTitle">Add New Lead Type</h3>
                <button type="button" onclick="closeModal('leadTypeModal')" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-rose-50 text-slate-400 hover:text-rose-500 transition-all">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Full Lead Type Name</label>
                    <input type="text" name="name" id="leadType_name" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="" required>
                </div>
                ...
                {{-- <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="action" id="leadType_status" class="sr-only peer" value="0" checked>
            ...
            <div class="mt-8 flex gap-3">
                <button type="button" onclick="closeModal('leadTypeModal')" class="flex-1 py-2.5 font-black text-slate-400 uppercase text-[10px] tracking-widest">Cancel</button>
...
<script>
    $(document).ready(function () {
        ...
        let editMode = false;
        const table = $('#lead-type-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('masters.lead-types.index') }}",
        ...
        window.openAddModal = () => {
            ...
            $('#leadType_id').val('');
            $('#modalTitle').text('Add New Lead Type');
            $('#leadTypeModal').removeClass('pointer-events-none opacity-0').addClass('opacity-100 bg-slate-900/30 backdrop-blur-sm');
        ...
        $('#floating-edit').click(function() {
            ...
            editMode = true;
            $.get(`{{ url('') }}/masters/lead-types/${id}/edit`, function(data) {
                $('#modalTitle').text('Update Lead Type');
                $('#leadType_id').val(data.id);
                $('#leadType_name').val(data.name);
                // $('#leadType_status').prop('checked', data.action == 0);
                $('#leadTypeModal').removeClass('pointer-events-none opacity-0').addClass('opacity-100 bg-slate-900/30 backdrop-blur-sm');
                $('.modal-content').removeClass('scale-95 opacity-0').addClass('scale-100 opacity-100');
            });
        });
        ...
        $('#leadTypeForm').submit(function(e) {
            ...
            const url = editMode
                ? `{{ url('') }}/masters/lead-types/${id}`
                : "{{ route('masters.lead-types.store') }}";
      ...
      window.handleBulkDelete = () => {
        ...
        if(!confirm(`Are you sure you want to delete ${ids.length} selected lead type(s)?`)){
            return;
        }

        $.ajax({
            url: "{{ route('masters.lead-types.bulkDelete') }}",
            type: "POST",
            data: { ids: ids },
