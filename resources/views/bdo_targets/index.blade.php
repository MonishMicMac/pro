...
<div class="flex-1 overflow-y-auto p-5 space-y-4 pb-20 bg-[#f8fafc]">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <span class="text-blue-600">BDO Targets</span>
            </nav>
    ...
    <div class="glass-panel rounded-[1.5rem] p-4">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
             <!-- Simplified Filters for brevity, can expand -->
            <div>
             ...
             <!-- JS will populate ZSM/BDM/BDO similar to other reports if needed -->
            <div>
            ...
            <table class="w-full" id="targetsTable">
                <thead>
                    <tr>
                         ...
                         <th colspan="3" class="pb-2 text-center text-[9px] font-black text-emerald-600 bg-emerald-50/50 rounded-t-lg mx-1">CONVERSION (SQFT)</th>
...
<div id="targetModal" class="hidden fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/0 backdrop-blur-0 transition-all duration-300 opacity-0 pointer-events-none">
    <div class="modal-content glass-panel w-full max-w-[600px] rounded-[1.25rem] p-6 shadow-2xl transition-all duration-300 transform scale-95 opacity-0 overflow-y-auto">
         <div class="flex items-start justify-between mb-5">
            <div>
                <h2 id="modalTitle" class="text-lg font-bold text-slate-800">Set Monthly Target</h2>
            </div>
            <button onclick="closeModal('targetModal')" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-rose-50 text-slate-400 hover:text-rose-500 transition-all">
        ...
        <form id="targetForm" class="space-y-4">
             ...
             <div class="grid grid-cols-2 gap-4">
                 <div>
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">BDO</label>
                    <select name="user_id" id="target_user_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required>
                  ...
                  <div>
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Month</label>
                    <input type="month" name="target_month" id="target_month" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required>
                 </div>
             </div>
             
             <div class="grid grid-cols-3 gap-4">
                  <div>
                    <label class="block text-[9px] font-black text-blue-600 uppercase tracking-widest mb-1.5">New Calls</label>
                    <input type="number" name="target_new_calls" id="target_new_calls" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required placeholder="0">
                 </div>
                  <div>
                    <label class="block text-[9px] font-black text-fuchsia-600 uppercase tracking-widest mb-1.5">Quotations</label>
                    <input type="number" name="target_quotations" id="target_quotations" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required placeholder="0">
                 </div>
                  <div>
                    <label class="block text-[9px] font-black text-amber-600 uppercase tracking-widest mb-1.5">Follow-ups</label>
                    <input type="number" name="target_followups" id="target_followups" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required placeholder="0">
                 </div>
             </div>
             
              <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label class="block text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-1.5">Conversion (Sqft)</label>
                    <input type="number" step="0.01" name="target_conversion_sqft" id="target_conversion_sqft" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required placeholder="0.00">
                 </div>
                  <div>
                    <label class="block text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1.5">Sales Value (₹)</label>
                    <input type="number" step="0.01" name="target_sales_value" id="target_sales_value" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required placeholder="0.00">
                 </div>
             </div>
             ...
             <div class="mt-8 flex gap-3">
                ...
                <button type="submit" class="flex-1 py-2.5 bg-slate-900 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-slate-900/20">Save Target</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    $('#targetForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function(err) {
                 Toast.fire({ icon: 'error', title: 'Error saving target' });
            }
        });
    });
</script>
