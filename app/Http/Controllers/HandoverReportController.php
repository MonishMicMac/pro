...
{
    /**
     * Display the handover site report landing page.
     */
    public function index()
    {
        $zones = Zone::where('action', '0')->orderBy('name')->get();
        $fabricators = Fabricator::where('status', '1')->orderBy('shop_name')->get();
        return view('reports.handover_report', compact('zones', 'fabricators'));
    }
    ...
    /**
     * Yajra DataTable endpoint for handover records.
     */
    public function data(Request $request)
    {
        ...
        $query = Lead::with(['assignedUser', 'fabricator'])
            ->whereNotNull('handovered_date')
            ->whereBetween('handovered_date', [$startDate, $endDate]);
        ...
        return DataTables::of($query)
            ...
            ->editColumn('handovered_date', function($row) {
                return $row->handovered_date ? Carbon::parse($row->handovered_date)->format('d-M-Y') : '-';
            })
