...
@section('title', 'Fabricator Collection Report')
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <span class="text-blue-600">Fabricator Collections</span>
            </nav>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Fabricator Collection Report</h1>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-none overflow-hidden relative z-10">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 id="detail-title" class="text-lg font-black text-slate-800 tracking-tight">Collection Records</h2>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-[11px]" id="collection-report-table">
                    <thead>
                        <tr class="text-left border-b border-slate-100">
                            <th class="pl-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Inv Date</th>
                            ...
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-right">Inv Amount</th>
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Due Date</th>
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-right">Collected</th>
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-right">Due Amt</th>
                            <th class="pr-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">Overdue Days</th>
                        </tr>
                    </thead>
...
<script>
    $(document).ready(function() {
        const table = $('#collection-report-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('fabricator.report.collections.data') }}",
                data: function (d) {
            ...
            columns: [
                ...
                { data: 'invoice_amount', name: 'invoice_amount', className: 'text-right' },
                { data: 'due_date', name: 'due_date' },
                { data: 'collected_amount', name: 'collected_amount', className: 'text-right text-emerald-600' },
                { data: 'due_amount', name: 'due_amount', className: 'text-right text-rose-600' },
                { 
                    data: 'overdue_days', 
                    name: 'overdue_days', 
                    className: 'text-center',
                    render: function(data) {
                        if (data <= 0) return `<span class="text-slate-300">-</span>`;
                        return `<span class="px-2 py-0.5 rounded-[4px] bg-rose-50 text-rose-600 text-[10px]">${data} Days</span>`;
                    }
                },
            ],
            order: [[0, 'desc']],
