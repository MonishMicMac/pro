...
<div class="relative flex-1 p-6 space-y-4 pb-24 bg-[#f8fafc] min-h-screen">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                <span>Reports</span> <span class="material-symbols-outlined text-[12px]">chevron_right</span> <span class="text-blue-600">BDO Joint Visits</span>
            </nav>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Joint Work Report</h1>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-[1.5rem] p-6 mb-6">
        <div class="flex flex-wrap gap-4 items-end">
            ...
            {{-- BDM Filter (Optional) --}}
            <div class="flex flex-col gap-1">
                <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Partner (BDM)</label>
            ...
            <table class="w-full" id="joint-table">
                <thead>
                    <tr class="text-left text-[10px] font-black text-slate-400 uppercase tracking-wider">
                        ...
                        <th class="px-4 pb-2 text-indigo-600">Partner (BDM)</th>
...
$(document).ready(function() {
    ...
    table = $('#joint-table').DataTable({
        ...
        columns: [
            ...
            { data: 'partner_name', name: 'bdm.name', className: 'text-indigo-600 font-bold' },
            { data: 'client_type', name: 'visit_type' },
            { data: 'client_name', name: 'client_name', orderable: false },
            { data: 'location', name: 'location', orderable: false },
            { data: 'status', name: 'action', className: 'text-right' }
        ],
        dom: 'rtp', 
        language: {
            paginate: { 
