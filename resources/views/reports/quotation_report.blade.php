    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <span class="text-blue-600">Quotation Analytics</span>
            </nav>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Quotation Report</h1>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-none overflow-hidden relative z-10">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-lg font-black text-slate-800 tracking-tight">Quotation Records</h2>
            ...
            <div class="overflow-x-auto">
                <table class="w-full" id="quotation-report-table">
                    <thead>
                        <tr class="text-left">
                            ...
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Contact</th>
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Lead Source</th>
                            ...
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">Rate</th>
                            <th class="pr-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">PDF</th>
                        </tr>
                    </thead>
...
<script>
    $(document).ready(function() {
        const table = $('#quotation-report-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('quotation.report.data') }}",
                data: function (d) {
            ...
            columns: [
                ...
                { data: 'quotation_date', name: 'created_at' },
                { data: 'lead_name', name: 'lead.name' },
                { data: 'phone_number', name: 'lead.phone_number' },
                { data: 'lead_source', name: 'lead.lead_source' },
                { data: 'fabricator_name', name: 'fabricator.shop_name' },
                { data: 'approx_sqft', name: 'approx_sqft', className: 'text-center' },
                { data: 'rate_per_sqft', name: 'rate_per_sqft', className: 'text-center' },
                { data: 'quotation_pdf', name: 'quotation_pdf', orderable: false, className: 'text-center' }
            ],
            order: [[1, 'desc']],
            dom: 'rtp',
            pageLength: 20,
        ...
        async function fetchLocationData(type, id) {
            try {
                const response = await fetch(`{{ route('quotation.report.location-data') }}?type=${type}&id=${id}`);
