    ...
    <div class="glass-panel rounded-[1.5rem] p-4">
        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4">
            <div>
                ...
                <select id="filter_lead_stage" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all">
            ...
            <div>
                ...
                <select id="filter_district_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:border-blue-500 transition-all">
            ...
            <div>
                ...
                <select id="filter_city_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:border-blue-500 transition-all">
    ...
    $(document).ready(function() {
        ...
        function getDistricts(stateId, callback) {
            ...
            $.get("{{ route('get.location.data') }}", { type: 'state', id: stateId }, function(data) {
                ...
                $.each(data, function(i, item) {
                    $('#filter_district_id').append(`<option value="${item.id}">${item.name}</option>`);
                });
                if(callback) callback();
            });
        }
        ...
        function getCities(districtId, callback) {
            ...
            $.get("{{ route('get.location.data') }}", { type: 'district', id: districtId }, function(data) {
                clearDropdown('#filter_city_id', 'Cities');
                $.each(data, function(i, item) {
                    $('#filter_city_id').append(`<option value="${item.id}">${item.name}</option>`);
                });
                if(callback) callback();
            });
        }
