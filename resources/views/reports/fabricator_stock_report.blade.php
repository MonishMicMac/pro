    ...
    <div class="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 p-5">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="date" id="from_date" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-blue-500 transition-all" 
                    value="{{ date('Y-m-01') }}">
            </div>
            <div>
                 <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">To Date</label>
                <input type="date" id="to_date" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-blue-500 transition-all" 
                    value="{{ date('Y-m-t') }}">
            </div>
            ...
            <div>
                ...
                <select id="zone_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-blue-500 transition-all">
            ...
            <div>
                ...
                <select id="fabricator_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-blue-500 transition-all">
                    <option value="">All Fabricators</option>
                    @foreach($fabricators as $fabricator)
                        <option value="{{ $fabricator->id }}">{{ $fabricator->shop_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                 ...
                 <select id="category_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-blue-500 transition-all">
            ...
            <div>
                 ...
                 <select id="sub_category_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-blue-500 transition-all">
                    <option value="">All Sub Categories</option>
                </select>
            </div>

             <div>
                 <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Product</label>
                 <select id="product_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:border-blue-500 transition-all">
    ...
    <div class="glass-panel rounded-[1.5rem] overflow-hidden">
         ...
         <div class="px-3 overflow-x-auto">
            <table class="w-full" id="report-table">
                <thead>
                    <tr class="text-left">
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider pl-4 whitespace-nowrap w-24">Last Updated</th>
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider whitespace-nowrap">Zone</th>
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider whitespace-nowrap">Fabricator</th>
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider whitespace-nowrap">Category</th>
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider whitespace-nowrap">Sub Category</th>
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider w-64">Product</th>
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center whitespace-nowrap">Current Stock</th>
                         <!-- Removed Opening/Closing as per request column focus -->
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider pr-4 whitespace-nowrap">Updated By</th>
                    </tr>
                </thead>
...
<script>
    $(document).ready(function() {
        ...
        const table = $('#report-table').DataTable({
            ...
            ajax: {
                ...
                data: function(d) {
                    ...
                    d.fabricator_id = $('#fabricator_id').val();
            ...
            columns: [
                { data: 'date', name: 'created_at', className: 'whitespace-nowrap' },
                { data: 'fabricator_zone', name: 'fabricator.zone.name', orderable: false, className: 'whitespace-nowrap' },
                { data: 'fabricator_name', name: 'fabricator.shop_name', className: 'whitespace-nowrap' },
                
                { data: 'category_name', name: 'product.category.name', defaultContent: '-', className: 'whitespace-nowrap' },
                { data: 'sub_category_name', name: 'product.subCategory.name', defaultContent: '-', className: 'whitespace-nowrap' },
                { data: 'product_name', name: 'product.name' }, // Allow wrapping for product
                
                { data: 'current_stock', name: 'current_stock', className: 'text-center' },
                { data: 'updated_by', name: 'updated_by', className: 'whitespace-nowrap' },
            ],
        ...
        $('#from_date, #to_date, #zone_id, #fabricator_id, #category_id, #sub_category_id, #product_id').change(function() { table.draw(); });
        ...
        $('#reset-filters').click(function() {
            ...
            $('#fabricator_id').html('<option value="">All Fabricators</option>');
            table.draw();
        });
        ...
        $('#category_id').change(function() {
            ...
            if(catId) {
                $.get("{{ route('fabricator-stock.report.sub-categories') }}", { category_id: catId }, function(data) {
                    let subs = data;
        ...
        $('#sub_category_id, #category_id').change(function() {
             ...
             if($(this).attr('id') === 'category_id') return; // Handled by category change resetting product
             
             if(catId && subId) {
                 prodSelect.html('<option value="">Loading...</option>');
                 $.get("{{ route('fabricator-stock.report.products') }}", { category_id: catId, sub_category_id: subId }, function(data) {
                     let prods = data;
                     let html = '<option value="">All Products</option>';
                     prods.forEach(p => {
                        // Assuming product object has 'item_code' or 'name'
                        html += `<option value="${p.id}">${p.name} (${p.item_code})</option>`;
                     });
        ...
        $('#zone_id').change(function() {
            let zoneId = $(this).val();
            if(!zoneId) return;
            $.get("{{ route('fabricator.report.location-data') }}", { type: 'zone', id: zoneId }, function(res) {
                let html = '<option value="">All Fabricators</option>';
                if(res.fabricators) {
                    res.fabricators.forEach(f => {
                        html += `<option value="${f.id}">${f.name}</option>`;
                    });
                }
                $('#fabricator_id').html(html);
            });
        });
    });
</script>
