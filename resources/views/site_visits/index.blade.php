...
<style type="text/tailwindcss">
    @layer components {
        ...
        /* Ensure dropdowns and inputs are clickable and styled */
        .form-input-custom {
    ...
    /* Force high z-index for form controls if context issues arise */
    .force-click {
        position: relative;
        z-index: 50;
    }
...
<div class="flex-1 overflow-y-auto p-5 space-y-4 pb-20 bg-[#f8fafc] relative z-0">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <span class="text-blue-600">Site Visits</span>
            </nav>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Site Visits</h1>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-[1.5rem] overflow-hidden relative z-10">
        <div class="px-4 overflow-x-auto pt-4">
            <table class="w-full" id="site-visit-table">
                <thead>
                    <tr class="text-left">
                        ...
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Lead</th>
                        ...
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Remarks</th>
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">Image</th>
...
<div id="imageModal" class="fixed inset-0 z-[9999] hidden flex items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-all duration-300 opacity-0" onclick="closeModal('imageModal')">
    <div class="modal-content relative transition-all duration-300 transform scale-95">
        <img id="modalImage" src="" class="max-w-[90vw] max-h-[90vh] rounded-xl shadow-2xl border-4 border-white" alt="Visit Image">
...
<script>
    $(document).ready(function() {
            ...
            columns: [
                ...
                { 
                    ...
                    render: function(data) {
                        return data ? `<button onclick="openImageModal('${data}')" class="w-8 h-8 rounded-lg overflow-hidden border border-slate-200 shadow-sm hover:scale-110 transition-transform"><img src="${data}" class="w-full h-full object-cover"></button>` : '<span class="text-slate-300">-</span>';
                    }
                },
                { 
                    data: 'map_data', 
        ...
        $('#filter_user_id, #filter_from_date, #filter_to_date').on('change', function() {
        ...
        window.openMapModal = (dataEncoded) => {
            ...
            if (data.lat && data.lng) {
                const m = L.marker([data.lat, data.lng]).addTo(map).bindPopup("Check In");
                markers.push(m);
                bounds.push([data.lat, data.lng]);
            }
            if (data.check_out_lat && data.check_out_lng) {
                const m = L.marker([data.check_out_lat, data.check_out_lng]).addTo(map).bindPopup("Check Out");
                markers.push(m);
