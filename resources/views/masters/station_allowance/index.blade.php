            ...
            <div class="p-4 bg-white/30 border-b border-white/50 flex justify-between items-center">
                <div class="relative w-full max-w-xs">
                    ...
                    <input id="customSearch"
                        ...
                        placeholder="Search allowances..." type="text" />
                </div>
            </div>
                ...
                <table class="w-full" id="station-allowance-table">
                    <thead>
                        <tr class="text-left">
                            ...
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                                Role
                            </th>

                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Station
                                Type</th>
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider text-end">
                                Amount</th>
                            <th
        ...
        <div
            ...
            <form id="stationAllowanceForm">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button type="button" onclick="closeModal('stationAllowanceModal')"
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="role_id" id="stationAllowance_role_id"
                    ...
                    <div>
                        <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Station
                            Type</label>
                        <select name="station_type" id="stationAllowance_station_type"
                    ...
                    <div>
                        <label
                            class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Amount</label>
                        <input type="number" step="0.01" name="amount" id="stationAllowance_amount"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all"
    ...
    <script>
        $(document).ready(function() {
            ...
            window.openAddModal = () => {
                ...
                $('#stationAllowanceModal')
                    .removeClass('pointer-events-none opacity-0 bg-slate-900/0 backdrop-blur-0')
                    .addClass('opacity-100 bg-slate-900/30 backdrop-blur-sm');

                $('#stationAllowanceModal .modal-content')
            ...
            window.closeModal = (id) => {
                ...
                // Fully reset backdrop
                $modal
                    .removeClass('opacity-100 bg-slate-900/30 backdrop-blur-sm')
                    .addClass('opacity-0 pointer-events-none bg-slate-900/0 backdrop-blur-0');
            };
            ...
            $('#floating-edit').click(function() {
                ...
                $.get(`{{ url('') }}/masters/station-allowance/${id}/edit`, function(data) {
                    ...
                    $('#stationAllowance_role_id').val(data.role_id);
                    $('#stationAllowance_station_type').val(data.station_type);
                    $('#stationAllowance_amount').val(data.amount);
                    $('#stationAllowanceModal').removeClass('pointer-events-none opacity-0')
                        .addClass('opacity-100 bg-slate-900/30 backdrop-blur-sm');
                    $('.modal-content', '#stationAllowanceModal').removeClass('scale-95 opacity-0')
                        .addClass('scale-100 opacity-100');
                });
            });
            ...
            $('#stationAllowanceForm').submit(function(e) {
                ...
                const url = editMode ? `{{ url('') }}/masters/station-allowance/${id}` :
                    "{{ route('masters.station-allowance.store') }}";
            ...
            $('#stationAllowanceModal').on('click', function(e) {
                if (e.target === this) {
                    closeModal('stationAllowanceModal');
                }
            });
