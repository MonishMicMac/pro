...
@section('title', 'Installation Site Report')
...
<style type="text/tailwindcss">
    @layer components {
        ...
        .form-input-custom {
            @apply w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all cursor-pointer;
        }
    }
    ...
    #report-pagination .paginate_button.current { @apply bg-emerald-600 text-white shadow-md border-emerald-600; }
...
<div class="flex-1 overflow-y-auto p-5 space-y-6 pb-20 bg-[#f8fafc] relative z-0">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <span class="text-emerald-600">Installation Analytics</span>
            </nav>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Installation Site Report</h1>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-none p-5 relative z-20">
        ...
        <div class="mt-4 flex justify-end gap-2">
            ...
            <button id="btn_filter" class="px-8 h-[42px] bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 active:scale-95 shadow-lg shadow-emerald-500/20">
    ...
    <div class="glass-panel rounded-none overflow-hidden relative z-10">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-lg font-black text-slate-800 tracking-tight">Installation Records</h2>
                    <p id="table-info" class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Total Records: 0</p>
                </div>
                <div class="flex items-center gap-4">
                    <div class="relative">
                        <input type="text" id="custom-search" placeholder="Search..." class="bg-white/50 border-slate-200 rounded-xl text-xs font-bold text-slate-600 placeholder:text-slate-300 w-64 pr-10 focus:ring-emerald-500">
            ...
            <div class="overflow-x-auto">
                <table class="w-full" id="installation-report-table">
                    <thead>
                        <tr class="text-left">
                            ...
                            <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Installed Date</th>
...
<script>
    $(document).ready(function() {
        const table = $('#installation-report-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('installation_site.report.data') }}",
                data: function (d) {
            ...
            columns: [
                ...
                { data: 'installed_date', name: 'installed_date' },
        ...
        async function fetchLocationData(type, id) {
            try {
                const response = await fetch(`{{ route('installation_site.report.location-data') }}?type=${type}&id=${id}`);
