...
<div class="flex-1 overflow-y-auto p-5 space-y-4 pb-20 bg-[#f8fafc]">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <nav class="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
                ...
                <span class="text-blue-600">Station Allowance</span>
            </nav>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Station Allowance Master</h1>
        </div>
        @can('station-allowance.create')
        <button onclick="openAddModal()" class="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all flex items-center gap-2">
            <span class="material-symbols-outlined text-[18px] font-bold">add</span> Add Station Allowance
        </button>
        @endcan
    </div>
    ...
    <div class="glass-panel rounded-[1.5rem] overflow-hidden">
        ...
        <div class="px-4 overflow-x-auto">
            <table class="w-full" id="station-allowance-table">
                <thead>
                    <tr class="text-left">
                        ...
                        <th class="px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-wider">Station Type</th>
...
<div id="stationAllowanceModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/0 backdrop-blur-0 transition-all duration-300 opacity-0 pointer-events-none">
    <div class="modal-content glass-panel w-full max-w-sm rounded-[1.25rem] p-6 shadow-2xl transition-all duration-300 transform scale-95 opacity-0">
        <form id="stationAllowanceForm">
            @csrf
            <input type="hidden" name="_method" id="method">
            <input type="hidden" id="stationAllowance_id" name="id">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg font-black text-slate-800" id="modalTitle">Add New Station Allowance</h3>
                <button type="button" onclick="closeModal('stationAllowanceModal')" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-rose-50 text-slate-400 hover:text-rose-500 transition-all">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Station Type</label>
                    <select name="station_type" id="stationAllowance_station_type" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" id="stationAllowance_amount" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="0.00" required>
                </div>
            </div>

            <div class="mt-8 flex gap-3">
                <button type="button" onclick="closeModal('stationAllowanceModal')" class="flex-1 py-2.5 font-black text-slate-400 uppercase text-[10px] tracking-widest">Cancel</button>
...
<script>
    $(document).ready(function () {
        ...
        let editMode = false;
        const table = $('#station-allowance-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('masters.station-allowance.index') }}",
        ...
        window.openAddModal = () => {
            editMode = false;
            $('#stationAllowanceForm')[0].reset();
            $('#stationAllowance_id').val('');
            $('#modalTitle').text('Add New Station Allowance');
            $('#stationAllowanceModal').removeClass('pointer-events-none opacity-0').addClass('opacity-100 bg-slate-900/30 backdrop-blur-sm');
            $('.modal-content', '#stationAllowanceModal').removeClass('scale-95 opacity-0').addClass('scale-100 opacity-100');
        };
        ...
        $('#floating-edit').click(function() {
            ...
            editMode = true;
            $.get(`{{ url('') }}/masters/station-allowance/${id}/edit`, function(data) {
                $('#modalTitle').text('Update Station Allowance');
                $('#stationAllowance_id').val(data.id);
                $('#stationAllowance_station_type').val(data.station_type);
                $('#stationAllowance_amount').val(data.amount);
                $('#stationAllowanceModal').removeClass('pointer-events-none opacity-0').addClass('opacity-100 bg-slate-900/30 backdrop-blur-sm');
                $('.modal-content', '#stationAllowanceModal').removeClass('scale-95 opacity-0').addClass('scale-100 opacity-100');
            });
        });

        $('#stationAllowanceForm').submit(function(e) {
            e.preventDefault();
            const id = $('#stationAllowance_id').val();
            $('#method').val(editMode ? 'PUT' : 'POST');
            const url = editMode ? `{{ url('') }}/masters/station-allowance/${id}` : "{{ route('masters.station-allowance.store') }}";
            ...
            $.ajax({
                ...
                data: $(this).serialize(),
                success: function() {
                    closeModal('stationAllowanceModal');
                    table.draw(false);
        ...
        window.handleBulkDelete = () => {
            ...
            $.ajax({
                url: "{{ route('masters.station-allowance.bulkDelete') }}",
                type: "POST",
                data: { ids: ids },
                success: function () {
                    table.draw(false);
