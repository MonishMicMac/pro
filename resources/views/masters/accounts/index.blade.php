...
<style type="text/tailwindcss">
    ...
    #accountModal.pointer-events-none { visibility: hidden; }
</style>
    ...
    <div class="glass-panel rounded-[1.5rem] overflow-hidden">
        <div class="p-4 bg-white/30 border-b border-white/50 flex justify-between items-center">
            <div class="relative w-full max-w-xs">
                ...
                <input id="customSearch" class="w-full pl-9 pr-3 py-1.5 bg-white/80 border border-slate-200 rounded-xl text-xs outline-none" placeholder="Search account..." type="text"/>
            </div>
        </div>
...
<div id="accountModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/0 backdrop-blur-0 transition-all duration-300 opacity-0 pointer-events-none">
    <div class="modal-content glass-panel w-full max-w-sm rounded-[1.25rem] p-6 shadow-2xl transition-all duration-300 transform scale-95 opacity-0">
        <form id="accountForm">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <button type="button" onclick="closeModal('accountModal')" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-rose-50 text-slate-400 hover:text-rose-500 transition-all">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-1">
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Account Name</label>
                    <input type="text" name="name" id="account_name" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required>
                </div>
                <div class="col-span-1">
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Mobile Number</label>
                    <input type="text" name="mobile_number" id="account_mobile" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required>
                </div>
                <div class="col-span-1">
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Account Type</label>
                    <select name="account_type_id" id="account_type_id" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required>
                ...
                <div class="col-span-1">
                    ...
                    <select name="zone_id" id="account_zone" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required>
                ...
                <div class="col-span-1">
                    ...
                    <select name="state_id" id="account_state" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required disabled>
                        <option value="">Select State</option>
                    </select>
                </div>
                <div class="col-span-1">
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">District</label>
                    <select name="district_id" id="account_district" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all" required disabled>
                        <option value="">Select District</option>
                    </select>
                </div>
                <div class="col-span-1">
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Pincode</label>
                    <input type="text" name="pincode" id="account_pincode" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all">
                </div>
                <div class="col-span-2">
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Address</label>
                    <textarea name="address" id="account_address" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-xs font-bold text-slate-700 focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all"></textarea>
                </div>
            </div>
            </div>

            <div class="mt-8 flex gap-3">
                <button type="button" onclick="closeModal('accountModal')" class="flex-1 py-2.5 font-black text-slate-400 uppercase text-[10px] tracking-widest">Cancel</button>
    ...
    $(document).ready(function () {
        ...
        $('#floating-edit').click(function() {
            ...
            $.get(`{{ url('') }}/masters/accounts/${id}/edit`, function(data) {
                ...
                // Wait for dependent dropdowns to load before setting values
                setTimeout(() => { 
                    $('#account_state').val(data.state_id).trigger('change');
                    setTimeout(() => {
                        $('#account_district').val(data.district_id);
                    }, 500);
                }, 500);

                $('#account_pincode').val(data.pincode);
        ...
        $('#account_zone').change(function() {
            ...
            if (id) {
                $.get(`{{ route('get.location.data') }}`, { type: 'zone', id: id }, function(data) {
                    data.forEach(item => {
        ...
        $('#account_state').change(function() {
            ...
            if (id) {
                $.get(`{{ route('get.location.data') }}`, { type: 'state', id: id }, function(data) {
                    data.forEach(item => {
        ...
        $('#accountForm').submit(function(e) {
            ...
            const url = editMode ? `{{ url('') }}/masters/accounts/${id}` : "{{ route('masters.accounts.store') }}";
            ...
            $.ajax({
                ...
                success: function() {
                    ...
                    table.draw(false);
                    Toast.fire({ icon: 'success', title: editMode ? 'Account Updated' : 'Account Created' });
                    resetFloatingBar();
                }
            });
        });
